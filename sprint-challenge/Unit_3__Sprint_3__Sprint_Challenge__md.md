...
### Part 1 - If I could put Flask in a File: The Setup
...
## Set this up in a file called aq_dashboard.py:
...
Visit the given URL (http://127.0.0.1:5000/) in your browser to verify that it works.
...
## Using a Database for Data Science
...
Create a `Record` model - you can start by copying the
...
list of tuples you created in part 2.

## To Do:
Use that logic/list, and add all the pulled records to the database. 
As the `Record` class indicates, store the
...
Also, while the provided `Record` class is adequate for storage, you **should** (is that required or stretch goal? *Should has a varying definition of both requirement and suggestion*)
...
## Functionality
To test and verify, you can execute`FLASK_APP=aq_dashboard.py flask shell` in terminal:
...
**Hint - your query should look like `Record.query.filter(condition).all()`, where `condition` is a
...
### Part 5 Finished - Turn it in!
Add `aq_dashboard.py` to your weekly repo
...
### Part 6 Stretch Goal (Optional) -  Put it on Heroku!
